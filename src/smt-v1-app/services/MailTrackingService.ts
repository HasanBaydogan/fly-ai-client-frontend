...
export const getAllRFQMails = async (pageNo : number, pageSize:number,filterType:string,sinceFromDate:string) => {
    try {
      ...
      const rfqMailResp = await api().post(`/rfq-mail/previews`,{
        pageNo:pageNo,
        pageSize:pageSize,
  ...
  export const searchByRfqNumberId = async (rfqNumberId:string,pageNo: number, pageSize: number) => {
    try {
      const accessToken = Cookies.get("access_token");
      const headers = {};
      if (accessToken) {
      ...
      const rfqMailResp = await api().get(`/rfq-mail/search/${rfqNumberId}/${pageNo}/${pageSize}`,{
        headers,
      });
