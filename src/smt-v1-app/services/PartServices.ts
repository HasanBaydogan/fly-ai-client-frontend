...
export const postUDFCreate = async (newUDF: createUDF) => {
  try {
    ...
    const response = await api().post(`/part/udf/create`, newUDF, {
      headers
    });
    // console.log('postUDFCreate Payload RESPONSE', response);
    ...
    console.error('postUDFCreate fonksiyonunda hata:', err);
  }
};
...
export const deleteByNotes = async (noteId: string) => {
  try {
    ...
    const response = await api().delete(`/part/note/note-id/${noteId}`, {
      headers
    });
    // console.log('Response from deleteByNotes:', response);
    ...
    console.log('[deleteByNotes] Permission Error:', err);
  }
};
...
export const getByDownloadFiles = async (partFileId: string) => {
  try {
    ...
    const response = await api().get(`/part-file/attachment/${partFileId}`, {
      headers
    });
    // console.log('Response from getByDownloadFiles:', response);
    ...
    if (response.data.statusCode === 200) {
      ...
      try {
        ...
        if (refreshTokenresponse.data.statusCode === 200) {
          ...
          let rfqMailResponseAfterRefresh = await api().get(
            `/part-file/attachment/${partFileId}`,
            {
              headers: {
                Authorization: `Bearer ${refreshTokenresponse.data.accessToken}`
              }
    ...
    console.log('[getByDownloadFiles] Permission Error:', err);
  }
};
...
export const getByAlternatives = async (partId: string, pageNo: number) => {
  try {
    ...
    console.log('[getByAlternatives ] Permission Error:', err);
  }
};
...
export const getByAlternativesDetail = async (partId: string) => {
  try {
    ...
    // console.log('Client getByAlternativesDetail partId', partId);

    const response = await api().get(
      `/part/alternative-part/history/id/${partId}`,
      {
        headers
      }
    );
    ...
    if (response.data.statusCode === 200) {
      ...
      try {
        ...
        if (refreshTokenresponse.data.statusCode === 200) {
          ...
          let rfqMailResponseAfterRefresh = await api().get(
            `/part/alternative-part/history/${partId}`,
            {
              headers: {
                Authorization: `Bearer ${refreshTokenresponse.data.accessToken}`
              }
    ...
    console.log('[getByAlternativesDetail] Permission Error:', err);
  }
};
